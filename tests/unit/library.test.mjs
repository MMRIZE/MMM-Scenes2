...
describe('Scenes library', () => {
  it('assigns defaults and one-based automatic scene names', async () => {
  ...
  it('hides exiting roles and shows entering roles', async () => {
    const modules = [
  ...
  it('follows named and callback-based branches', async () => {
  ...
  it('blocks next and previous when configured as false', async () => {
    const scenes = new Scenes({
      scenario: [{
        name: 'locked',
        next: false,
  ...
  it('dispatches public scene commands', async () => {
    const scenes = new Scenes({
  ...
  it('keeps the current scene when a branch target is unknown', async () => {
    const scenes = new Scenes({
      scenario: [
  ...
  it('rejects malformed numeric branch targets', async () => {
    const scenes = new Scenes({
      scenario: [
  ...
  it('supersedes a transition when another scene starts', async () => {
    const modules = [
      { classes: ['old'], hidden: false },
  ...
  it('handles an empty scenario without throwing', async () => {
    const scenes = new Scenes()
  ...
  it('advances when a scene lifetime expires', async (t) => {
  ...
  it('pauses and resumes the scene lifetime', async (t) => {
    mock.timers.enable({ apis: ['Date', 'setTimeout'], now: 0 })
    t.after(() => mock.timers.reset())
    const scenes = new Scenes({
      scenario: [
